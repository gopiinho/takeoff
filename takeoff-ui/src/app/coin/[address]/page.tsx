...
export default function Coin({ params }: { params: { address: string } }) {
  ...
  const tokenData = data as unknown as TokenInfoType
      ...
      {params.address ? (
        <div className="flex max-sm:flex-col justify-between px-4">
          <div className="flex gap-2 border border-transparent p-4">
            <div className="max-sm:w-28 max-sm:h-28 w-48 h-48 flex-shrink-0 bg-red-400" />
            <div className="flex flex-col gap-1 flex-1">
              <span className="max-sm:text-xs text-sm">created by: {tokenData.creator}</span>
              <span className="max-sm:text-xs text-xl text-green-400">raised: {tokenData.raised}/24 ETH</span>
              <div>
